...
export function SocialAuth({ service, onClick }: SocialAuthProps) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiHost}/auth/sso/${service}/authorize?redirect_uri=${encodeURIComponent(redirectUri)}`, {
        method: 'GET',
        headers: {
      ...
      setError(error instanceof Error ? error.message : 'Unknown authorization error');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="text-center">
            <Button 
              ...
              className={cn(
                ...
                service === 'google' && 'bg-white text-[#222] text-[15px] border-[#ccc] hover:bg-white/90',
                service === 'naver' && 'bg-[#03C75A] text-white text-[15px] border-none hover:bg-[#03C75A]/90'
              )}
              disabled={isLoading}
            >
